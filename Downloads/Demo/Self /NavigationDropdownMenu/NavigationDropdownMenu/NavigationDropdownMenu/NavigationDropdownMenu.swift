...
public class NavigationDropdownMenu: UIView {
    ...
    public init(navigationController: UINavigationController? = nil, containerView: UIView = UIApplication.sharedApplication().keyWindow!, title: String, items: [AnyObject]) {
        ...
        self.menuButton.addTarget(self, action: #selector(NavigationDropdownMenu.menuButtonTapped(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        self.tableView = TableView(frame: CGRectMake(menuWrapperBounds.origin.x, menuWrapperBounds.origin.y + 0.5, menuWrapperBounds.width, menuWrapperBounds.height + 300 - navBarHeight - statusBarHeight), items: items, title: title, configuration: self.configuration)
...
class Configuration {
    ...
    func defaultValue() {
        ...
        let url = bundle.URLForResource("NavigationDropdownMenu", withExtension: "bundle")
...
class TableView: UITableView, UITableViewDelegate, UITableViewDataSource {
    ...
    init(frame: CGRect, items: [AnyObject], title: String, configuration: Configuration) {
    ...
    override func hitTest(point: CGPoint, withEvent event: UIEvent?) -> UIView? {
        if let hitView = super.hitTest(point, withEvent: event) where hitView.isKindOfClass(TableCellContentView.self) {
            return hitView
        }
        return nil;
    }
...
//        CGContextStrokePath(context)
        ...
        let location = UIScreen.mainScreen().bounds.size.width-130
        CGContextMoveToPoint(ctx,location -  10 - 10, 180)
        CGContextAddLineToPoint(ctx,location - 2*10 - 10 ,  170)
        CGContextAddLineToPoint(ctx,location - 10 * 3 - 10, 180)
        CGContextClosePath(ctx)
        UIColor.whiteColor().setFill()  //设置填充色
        UIColor.whiteColor().setStroke() //设置边框颜色
        CGContextDrawPath(ctx,.FillStroke)//绘制路径path
    }
}
